...
use Exporter;
use vars qw(@EXPORT @ISA $VERSION $LENGTH $EXTENSION $DEBUG 
    $result $number_len $recover @FILE $num_files 
    $OFFSET $size_written $outfile @file_name @file_size 
    $result_size $filename
    $num_read);
@EXPORT =   qw( do_parity );
@ISA =      qw( Exporter ); 
